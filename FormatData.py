...
1. Format the training dataset containing all the diffusion image volumes and training lables, which allows you
    ...
    the size of inputs you use (voxels, 2d patches and 3d patches), choose different method: 
    
    python FormatData.py --path $DataDir --subjects TrainSubjID1  TrainSubjID2 --conv3d_train 
...
parser.add_argument("--fc1d_train", help="generate voxel-wise data for training", action="store_true")
...
patch_size = 3
label_size = 1
ltype = ['dmean', 'dax', 'drad', 'fa', 'kmean', 'kax', 'krad', 'kfa']
...
if test:
    for subject in subjects:
        ...
        if scheme is not None:
            ...
            nDWI = combine.sum()
        gen_dMRI_test_datasets(path, subject, nDWI, scheme, combine, ltype, fdata=True, flabel=False, whiten=True)
